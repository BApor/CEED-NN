...
object PytorchMobile {
    ...
    fun detect (context: Context, imageProxy: ImageProxy) : List<PostProcessor.DetectResult>{
        ...
        var boxes: IValue = IValue.from(0)
        var scores: IValue = IValue.from(0)

        try{
            val (_x, _boxes, _scores) = model.forward(IValue.from(inputTensor)).toTuple()
            x = _x
            boxes = _boxes
            scores = _scores
        } catch (exc: Exception) {
